...
const quizData = [
	...
	},
	{
		...
		"imageLink": '<img class="dog-pic" src="https://preview.ibb.co/b1f8LG/whippet.jpg" alt="whippet">',
		"option1": "Whippet" ,
		"option2": "Greyhound",
...
// These two arrays contain gifs for displaying on the final page.   
// A score > 6/10 will receive a success gif.
// Gifs within each array are chosen at random by loadResultsPageHtml().
const successGifs = 
	[
		{
			imageLink: 'https://media.giphy.com/media/l4FGzcWoiddHwXQQg/giphy.gif',
			alt: 'A man performs a trick shot at a pool table with a shih tzu standing on it'
		},
		{
	 		imageLink: 'https://media.giphy.com/media/dTJd5ygpxkzWo/giphy.gif',
	 		alt: 'A pug having his belly scratched'
		},
	 	{
	 		imageLink: 'https://media.giphy.com/media/1LweXxLwVT0J2/giphy.gif',
	 		alt: 'A dog with a hotdog in its mouth'
		},
	];
const failureGifs = 
	[
		{
			...
			alt: 'A dog with a stick in its mouth crashes into a fence'
		},
		{
	 		imageLink: "https://media.giphy.com/media/ZFZLIuWWkr29y/giphy.gif",
	 		alt: 'A dog face-plants into a hedge'
		},
		{
	 		imageLink: 'https://media.giphy.com/media/42075qeD6Mk0M/giphy.gif',
	 		alt: 'A frisbee is thrown at a bemused dog'
	 	}
	];

// questionCount tracks how many questions have been answered.  
// score tracks how many questions have been answered correctly.
let questionCount = 10, score = 0;
...
function giveFeedback(result) {
	...
	if (result) {
		...
		htmlToReplaceWithFeedback.text(`Sorry!`);
	}
}

// loadResultsPageHtml() is called when the user clicks 'next' on the final question.
...
function loadResultsPageHtml() {
	let finalGif;
	let finalWord;
	// The function randomly chooses a gif: a 'success' gif from the array 
	// successGifs or a 'failure' gif from failureGifs, depending on whether
	// the user scored more than 6/10).
	if (score > 6) {
		finalWord = "Congratulations!";
		finalGif = successGifs[Math.floor(Math.random()*3)];
	} else {
		finalWord = "Bad luck!";
		finalGif = failureGifs[Math.floor(Math.random()*3)];;
	}
...
`<h2 class="heading-success-failure">${score}/10<br>${finalWord}</h2>
			<img class="img-success-failure" src="https://media.giphy.com/media/l4FGzcWoiddHwXQQg/giphy.giff" alt="${finalGif.alt}">
		<br>
			<button name="Start again button" class="button-start-again">Play again <img class="paw-print" src="https://image.ibb.co/jFBR7w/paw_print.png" alt="paw_print"></button>`
		)
}
